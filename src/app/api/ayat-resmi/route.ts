...
import { fetchAyatResmi } from '@/lib/quran-api'

export async function GET(req: Request) {
  const { searchParams } = new URL(req.url)
  const surah = Number(searchParams.get('surah'))
  const ayat = Number(searchParams.get('ayat'))

  if (!surah || !ayat) {
    return NextResponse.json({ error: 'Parameter surah dan ayat wajib diisi' }, { status: 400 })
  }

  const data = await fetchAyatResmi(surah, ayat)
  if (!data) {
  ...
  return NextResponse.json({
    data: {
      ...
      surah_id: surah,
      surah_nama: surahNames[surah] || `Surah ${surah}`,
      ayat_num: ayat,
    }
  })
}
