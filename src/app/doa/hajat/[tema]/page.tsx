...
export default function DoaByHajatPage() {
  ...
  // Validasi tema
  const isValidTema = Object.keys(HAJAT_INFO).includes(tema)
  const info = isValidTema ? HAJAT_INFO[tema as TemaHajat] : null

  const doaList = useMemo(() => {
    if (!isValidTema) return []
    let list = getByHajat(tema as TemaHajat)
    ...
    return list
  }, [tema, isValidTema, searchQuery])

  if (!isValidTema || !info) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4">
        <h1 className="text-2xl text-[var(--gold)] mb-4">Tema hajat tidak ditemukan</h1>
        <button onClick={() => router.push('/doa/hajat')} className="text-[var(--teal-200)] flex items-center gap-2">
          <ArrowLeft className="w-4 h-4" /> Kembali ke Daftar Hajat
        </button>
      </div>
    )
  }
    ...
    <div className="min-h-screen pb-24 font-cairo">
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-[var(--dark3)] to-[var(--dark)] pt-16 pb-8 px-4 sm:px-6 lg:px-8 border-b border-[var(--gold-border)]">
        ...
        <div className="relative max-w-4xl mx-auto">
          <button onClick={() => router.push('/doa/hajat')} className="font-cairo flex items-center gap-2 text-[var(--text2)] hover:text-[var(--gold-light)] mb-6 transition-colors w-fit">
            <ArrowLeft className="w-4 h-4" /> Kembali ke Kategori Hajat
          </button>
          ...
          <p className="font-cairo text-sm text-[var(--text2)] mb-8 max-w-2xl">
            Menampilkan {doaList.length} doa dari Al-Qur&apos;an yang berkaitan dengan tema {info.label}.
          </p>
          ...
          <div className="relative max-w-xl">
            ...
            <input
              type="text"
              placeholder={`Cari dalam kategori ${info.label}...`}
              value={searchQuery}
              ...
              <Link key={doa.id} href={`/doa/${doa.id}`} className="block group">
                <div className="glass-card p-5 md:p-6 rounded-3xl transition-all duration-300 hover:shadow-[0_4px_20px_rgba(201,163,90,0.1)] hover:border-[var(--gold-light)]/50 relative overflow-hidden">
                  ...
                  <div className="flex items-center gap-2 mb-3 flex-wrap">
                    ...
                    <span className="font-cairo px-2.5 py-0.5 bg-[var(--dark3)] border border-[var(--gold-border)] rounded-full text-[10px] uppercase tracking-widest font-bold text-[var(--gold)]">
                      {doa.kategori}
                    </span>
                    {doa.nabi && (
