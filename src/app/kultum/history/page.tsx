      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 overflow-x-hidden">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
          <div className="relative w-full sm:w-64 shrink-0">
            ...
            <input
              ...
              className="font-cairo w-full bg-[var(--dark2)] border border-[var(--gold-border)] rounded-full pl-9 pr-4 py-2 text-sm text-[var(--text1)] placeholder-[var(--text3)] focus:outline-none focus:border-[var(--gold)]"
            />
          </div>
          <div className="flex gap-2 w-full sm:w-auto overflow-x-auto pb-2 sm:pb-0 hide-scrollbar pr-6">
            <button
              onClick={() => setFilter('semua')}
              className={`font-cairo px-4 py-2 rounded-full text-xs uppercase tracking-widest font-bold transition-all whitespace-nowrap border ${filter === 'semua' ? 'bg-[var(--gold)]/20 border-[var(--gold)] text-[var(--gold-light)]' : 'bg-[var(--dark2)] border-[var(--gold-border)] text-[var(--text2)]'}`}
            >
              Semua
            </button>
            <button
              onClick={() => setFilter('favorit')}
              className={`font-cairo px-4 py-2 rounded-full text-xs uppercase tracking-widest font-bold transition-all whitespace-nowrap border flex items-center gap-1 ${filter === 'favorit' ? 'bg-amber-500/20 border-amber-500/30 text-amber-400' : 'bg-[var(--dark2)] border-[var(--gold-border)] text-[var(--text2)]'}`}
            >
              <Star className="w-3 h-3" /> Favorit
            </button>
            <button
              onClick={() => setFilter('belum_dipakai')}
              className={`font-cairo px-4 py-2 rounded-full text-xs uppercase tracking-widest font-bold transition-all whitespace-nowrap border flex items-center gap-1 ${filter === 'belum_dipakai' ? 'bg-[var(--teal-500)]/20 border-[var(--teal-500)]/30 text-[var(--teal-300)]' : 'bg-[var(--dark2)] border-[var(--gold-border)] text-[var(--text2)]'}`}
            >
        ...
        {loading ? (
          ...
          <div className="text-center py-16 glass-card rounded-3xl border-[var(--gold-border)]">
            ...
            <p className="font-cairo text-[var(--text2)] text-sm mb-6">Anda belum pernah membuat kultum atau belum login.</p>
            <button onClick={() => router.push('/kultum')} className="font-cairo px-6 py-2 bg-gradient-to-r from-[var(--gold-light)] to-[var(--gold)] text-[var(--dark)] rounded-full text-sm font-bold shadow-lg">
              Buat Kultum Pertama
            </button>
          </div>
        ) : Object.keys(groupedHistory).length === 0 ? (
          <div className="text-center py-16 text-[var(--text2)] text-sm font-cairo">
          ...
          <div className="space-y-8">
            {Object.entries(groupedHistory).map(([month, items]) => (
              <div key={month} className="space-y-4">
                <h3 className="font-cairo text-sm font-bold text-[var(--teal-300)] uppercase tracking-widest pl-2 border-l-2 border-[var(--teal-500)]">
                  {month}
                </h3>
                      ...
                      <div className="glass-card p-5 rounded-2xl border-[var(--gold-border)] hover:border-[var(--teal-500)] transition-colors flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center overflow-hidden w-full">
                        ...
                        <div className="flex items-center gap-3 w-full sm:w-auto justify-end pt-3 sm:pt-0 border-t sm:border-0 border-[var(--dark3)] shrink-0">
                          ...
                          <Link
                            ...
                            className="font-cairo flex items-center gap-1 text-xs font-bold text-[var(--teal-200)] group-hover:translate-x-1 transition-transform"
                          >
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-[var(--dark2)] border border-rose-500/30 rounded-2xl p-6 max-w-sm w-full mx-4 shadow-2xl">
            <h3 className="font-cinzel text-lg font-bold text-[var(--text1)] mb-2">Hapus Kultum?</h3>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="font-cairo flex-1 py-2 rounded-xl border border-[var(--gold-border)] text-[var(--text2)] text-sm font-bold hover:bg-[var(--dark3)] transition-colors"
              >
                Batal
              </button>
              <button
                onClick={handleDelete}
                className="font-cairo flex-1 py-2 rounded-xl bg-rose-500 text-white text-sm font-bold hover:bg-rose-600 transition-colors"
              >
                Hapus
              </button>
            </div>
