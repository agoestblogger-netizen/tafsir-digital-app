...
function KultumGeneratorInner() {
  ...
  const handleGenerate = async () => {
    ...
    if (!temaFinal) {
      setError(isKisahMode ? 'Pilih kisah Al-Quran terlebih dahulu' : 'Pilih topik dan salah satu rekomendasi judul terlebih dahulu')
      return
    }
    ...
    try {
      ...
      if (!isKisahMode) {
        ...
        try {
          ...
          if (interleavedRes.ok) {
            const interleavedData = await interleavedRes.json()
            penjabaranInterleaved = interleavedData.penjabaran_interleaved ?? null
            console.log('[build-interleaved] done, length:', penjabaranInterleaved?.length)
          }
        } catch (e) {
          console.warn('[build-interleaved] gagal, fallback ke format biasa:', e)
          isInterleaved = false
        }
      }
      ...
      if (finalId) {
        // Simpan interleaved ke sessionStorage sebelum redirect ke [id]
        if (penjabaranInterleaved) {
          sessionStorage.setItem('kultum_penjabaran_interleaved', penjabaranInterleaved)
        } else {
          sessionStorage.removeItem('kultum_penjabaran_interleaved')
        }
        const redirectUrl = penjabaranInterleaved
          ? `/kultum/hasil/${finalId}?interleaved=1`
          : `/kultum/hasil/${finalId}`
        ...
        // Inject penjabaran_interleaved langsung ke dalam kultum_result
        if (penjabaranInterleaved) {
          try {
            const existing = JSON.parse(sessionStorage.getItem('kultum_result') || '{}')
            existing.__penjabaran_interleaved = penjabaranInterleaved
            sessionStorage.setItem('kultum_result', JSON.stringify(existing))
          } catch (e) {
            console.warn('[interleaved] gagal inject ke kultum_result:', e)
          }
        }
              ...
              {!isKisahMode && (
                <div className="mt-3 space-y-2">
                  <div className="flex gap-2">
                    {[
                      { label: '📖 Ayat', done: referensiDipilih.some((r: any) => r.type === 'ayat_quran_db'), required: true },
                      { label: '📜 Hadits', done: referensiDipilih.some((r: any) => r.type === 'hadits'), required: true },
                      { label: '🤲 Doa', done: referensiDipilih.some((r: any) => r.type === 'doa_quran'), required: false },
                      { label: '📝 Judul', done: selectedJudul !== '', required: true },
                    ].map((step, i) => (
                      <div key={i} className="flex-1 space-y-1.5">
                        <div className={`h-1.5 rounded-full transition-all duration-500 ${step.done ? 'bg-[#C9A84C]' : step.required ? 'bg-white/15' : 'bg-white/5'}`} />
                        <p className={`font-cairo text-sm font-semibold text-center transition-colors ${step.done ? 'text-[#C9A84C]' : step.required ? 'text-white/60' : 'text-white/30'}`}>
            ...
            <div className="pt-4">
              <button
                ...
                disabled={
                  ...
                  (isKisahMode 
                    ? !selectedKisah 
                    : (!selectedJudul || !referensiDipilih.some((r: any) => r.type === 'ayat_quran_db') || !referensiDipilih.some((r: any) => r.type === 'hadits')))
                }
                className="font-cairo w-full py-4 rounded-2xl bg-gradient-to-r from-[var(--gold-light)] to-[var(--gold)] text-[var(--dark)] font-bold text-xl hover:shadow-[0_0_24px_rgba(201,163,90,0.5)] transition-all flex items-center justify-center gap-3 disabled:opacity-40 disabled:cursor-not-allowed"
              >
                <Sparkles className="w-6 h-6" /> Generate {formatLabel} Sekarang
              </button>
