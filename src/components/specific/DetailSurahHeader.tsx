  ...
  return (
    <div className="flex flex-col gap-6 pt-6 pb-4">
      ...
      <div className="flex flex-col items-center text-center mt-2">
        <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-foreground mb-1">{surah.name}</h1>
        <p className="text-muted-foreground mb-6 text-xs md:text-sm tracking-widest uppercase">{surah.translation}</p>
        
        <p className="font-arabic text-4xl md:text-5xl text-primary font-medium mb-8 drop-shadow-sm">{surah.arab}</p>
        
        <div className="card-premium p-5 md:p-6 rounded-3xl relative overflow-hidden w-full text-left flex flex-col gap-3 md:gap-4">
          <div className="absolute top-0 right-0 w-32 h-32 bg-gold/10 rounded-full blur-3xl -mr-16 -mt-16 pointer-events-none" />
          <p className="text-foreground/80 leading-relaxed text-xs md:text-sm relative z-10 font-medium">
            &quot;{surah.pitch}&quot;
          </p>
          {surah.resume && (
            <>
              ...
              <div 
                className="text-foreground/70 leading-relaxed text-xs md:text-sm relative z-10 italic"
