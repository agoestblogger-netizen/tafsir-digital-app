  ...
  return (
    <Link href={`/surah/${surah.id}`}>
      <motion.div whileTap={{ scale: 0.96 }} whileHover={{ y: -2 }}>
        <Card className="hover:border-primary/50 transition-colors card-premium relative overflow-hidden group">
          ...
          <CardContent className="p-5">
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3">
                <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-secondary text-primary font-semibold">
                  {surah.id}
                </div>
                <div>
              ...
              <div className="text-right">
                <p className="font-arabic text-xl text-foreground font-medium">{surah.arab}</p>
              </div>
            </div>
